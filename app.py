...
st.title("EV Charging Station Cost Calculator")
...
if uploaded_file is not None:
    ...
    # Convert to Calgary time (Mountain Time)
    calgary_tz = pytz.timezone('America/Edmonton')
    df['Session Start'] = df['Session Start'].dt.tz_localize('UTC').dt.tz_convert(calgary_tz)
    df['Session End'] = df['Session End'].dt.tz_localize('UTC').dt.tz_convert(calgary_tz)
    
    min_date = df['Session Start'].dt.date.min()
    max_date = df['Session End'].dt.date.max()
    ...
    for name, group in df.groupby('QR Code Name'):
        ...
        for idx, row in group.iterrows():
            ...
            # Calculate paid idle hours only (outside 12am-7am)
            paid_idle_hours = 0
            current = idle_start
            while current + timedelta(hours=1) <= idle_end:
                next_hour = current + timedelta(hours=1)
                if not (time(0, 0) <= current.time() < time(7, 0)):
                    paid_idle_hours += 1
                current = next_hour
